                  ...
                  {layers.map((l) => (
                    <div className="print-legend-row" key={l.id}>
                      {l.kind === "raster" ? (
                        ...
                        // A plain CSS `background-color` on an empty <span>
                        // here was printing as blank/invisible in testing —
                        // the browser's print rasterizer dropped it even
                        // though other CSS backgrounds on the same page (the
                        // legend box itself) came through fine. An inline
                        // SVG shape with a `fill` is the same mechanism the
                        // north arrow above already relies on and prints
                        // correctly, so the swatch uses that instead.
                        <svg className="print-legend-swatch" width="11" height="11" viewBox="0 0 11 11">
                          <rect width="11" height="11" rx="3" fill={l.style.color} />
                        </svg>
                      )}
                      {l.name}
                    </div>
