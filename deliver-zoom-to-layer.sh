#!/usr/bin/env bash
# GISNEXUS — "Zoom to layer" button in the layer list, so a small result
# (like a Watershed polygon, which can be a tiny fraction of the current
# view) is findable with one click instead of manual panning/zooming.
#
# Frontend-only — no backend/API changes, nothing new to redeploy on Render.
#
# Run this from the ROOT of your gisnexus-app repo, in Git Bash:
#   bash deliver-zoom-to-layer.sh
set -e
...
echo "Writing apps/web/src/lib/geoBounds.ts ..."
cat > apps/web/src/lib/geoBounds.ts <<'EOF'
...
echo "  git add -A"
echo "  git commit -m \"Add zoom-to-layer button in the layer list\""
echo "  git push"