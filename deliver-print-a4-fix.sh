#!/usr/bin/env bash
# Pins the printed map to a true, fixed A4 page (single page, all elements
# included) regardless of what device/screen size generated the PDF, and
# makes the Portrait/Landscape toggle actually control the printed page
# orientation (previously it was hardcoded to landscape-only, and the sheet
# was sized as a percentage of the viewport — which is why the PDF came out
# a different size on mobile vs. desktop).
#
# Run this from the root of your gisnexus-app checkout:
#   bash deliver-print-a4-fix.sh
set -euo pipefail
...
echo "  git add -A"
echo "  git commit -m \"Pin printed map to a fixed A4 page with working portrait/landscape toggle\""
echo "  git push"