...
const MapCanvas = forwardRef<MapCanvasHandle, Props>(function MapCanvas(
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      // MapLibre's type for this option is `false | AttributionControlOptions`
      // (not a plain boolean) — `false` omits the control, `undefined` (the
      // non-print case) falls back to the library's own default of `true`.
