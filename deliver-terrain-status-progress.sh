#!/usr/bin/env bash
# Two UX fixes to the Terrain panel:
#  1. The status/error message now sticks to the top of the panel instead of
#     only appearing after Watershed at the very bottom — no more scrolling
#     all the way down to see whether something happened.
#  2. Running a tool now shows a progress bar with a percentage. Since
#     WhiteboxTools runs as a single request/response on the server (no
#     incremental progress is streamed back), this is a time-based estimate
#     meant to reassure you it's actually working, not a literal measurement
#     of how much of the DEM has been processed.
#
# Run this from the root of your gisnexus-app checkout:
#   bash deliver-terrain-status-progress.sh
set -euo pipefail
...
/* TerrainPanel's status/progress banner — sticky to the top of .bottom-
   content (its scrollable parent) so it stays visible while running a tool
   regardless of which tool section further down the panel is scrolled into
   view. The negative margin matches .bottom-content's own padding (14px 18px)
   so the banner bleeds edge-to-edge when pinned, instead of floating with a
   gap on either side. */
.terrain-status {
  position: sticky; top: 0; z-index: 5; margin: -14px -18px 14px; padding: 10px 18px;
  font-size: 12.5px; font-weight: 600; line-height: 1.5;
  backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px);
}
.terrain-status.busy { background: rgba(124,92,255,0.16); color: var(--brand-2); border-bottom: 1px solid rgba(124,92,255,0.3); }
.terrain-status.error { background: var(--danger-bg); color: #ff8fa4; border-bottom: 1px solid rgba(255,92,122,0.25); }
.terrain-progress-track { margin-top: 7px; height: 6px; border-radius: 4px; background: rgba(255,255,255,0.14); overflow: hidden; }
.terrain-progress-fill { height: 100%; border-radius: 4px; background: var(--gradient); transition: width 0.25s ease; }
.terrain-progress-pct { margin-top: 4px; font-size: 11px; font-weight: 700; color: var(--ink-soft); text-align: right; }
...
.print-toolbar-actions { display: flex; gap: 10px; align-items: center; flex-wrap: wrap; }
...
.print-orientation-field { display: flex; flex-direction: column; gap: 4px; font-size: 10.5px; font-weight: 700; color: var(--ink-soft); text-transform: uppercase; letter-spacing: .04em; }
.print-orientation-toggle { display: flex; border: 1px solid var(--line); border-radius: 7px; overflow: hidden; }
.print-orientation-toggle button {
  border: none; background: rgba(255,255,255,0.06); color: var(--ink-soft); font-size: 12.5px; font-weight: 700;
  text-transform: none; letter-spacing: normal; padding: 6px 12px;
}
.print-orientation-toggle button.active { background: var(--gradient); color: #fff; }

/* .print-sheet is rendered at its true, fixed A4 pixel size (see PAGE_PX in
   PrintMapModal.tsx) via inline width/height — NOT a percentage or viewport
   unit — so the printed output is identical no matter what device or screen
   size generated it. .print-preview-area/.print-sheet-scale-wrap exist
   purely to make that fixed-size sheet previewable on screen: the wrap is
   sized to the VISUALLY scaled dimensions with overflow hidden, and
   .print-sheet is shrunk into it with a CSS transform (paint-only, doesn't
   change its real layout size) — see the @media print overrides below,
   which turn the scaling back off for the actual print/PDF pass. */
.print-preview-area { display: flex; justify-content: center; padding: 28px 24px 40px; }
.print-sheet-scale-wrap { position: relative; overflow: hidden; flex-shrink: 0; box-shadow: 0 30px 80px rgba(0,0,0,0.55); }
.print-sheet {
  background: var(--white); border: 2px solid #171a2e; border-radius: 4px; padding: 22px;
  display: flex; flex-direction: column; transform-origin: top left;
}
.print-sheet-header, .print-sheet-footer { flex: 0 0 auto; }
...
/* flex: 1 (not a fixed height) — since .print-sheet is now pinned to an
   exact total page height, letting the map frame absorb whatever's left
   after the header/footer guarantees everything fits on the one page,
   regardless of how tall the legend or credits happen to be. */
.print-map-frame { position: relative; flex: 1 1 auto; min-height: 0; border: 1px solid #dfe3ee; overflow: hidden; }
...
@media print {
  /* @page's `size` (A4, portrait/landscape) is set dynamically from the
     user's toggle via the <style> tag PrintMapModal.tsx renders alongside
     .print-modal-backdrop — see that component for why it can't live here
     as a static rule. */
  ...
  .print-preview-area { display: block; padding: 0; }
  /* Turn the on-screen "shrink the true-size sheet to fit this device's
     viewport" trick back off for the actual print/PDF pass — a stylesheet
     `!important` DOES win over a plain (non-!important) inline style, so
     these beat the `transform`/width/height React sets inline for the
     on-screen preview. What's left is .print-sheet at its real, fixed
     A4-matching pixel size, identical on every device. */
  .print-sheet-scale-wrap { width: auto !important; height: auto !important; overflow: visible; box-shadow: none; }
  .print-sheet { transform: none !important; margin: 0; box-shadow: none; border-width: 1.5px; }
...
echo "  git add -A"
echo "  git commit -m \"Make terrain tool status/progress sticky and add a progress bar\""
echo "  git push"